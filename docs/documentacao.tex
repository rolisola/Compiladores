...
\title{Documentação de projeto mybc}
\author{Vitor Marchini Rolisola, André Luis Dias Nogueira, Lucas Abdala Martins}
\date{November 2025}
...
\begin{titlepage}
    ...
    \vfill
    \Huge\textbf{Documentação de projeto \\ mybc}\\[1.5cm]
...
\section*{Resumo de Funcionalidades}
\begin{itemize}
    \item Avaliação de expressões aritméticas infixas com `+`, `-`, `*`, `/`.
    \item Literais: decimal, octal (`0` prefixado), hexadecimal (`0x`/`0X`), ponto flutuante (inclui notação exponencial), numerais romanos clássicos com notação subtrativa.
    \item Atribuição com operador `:=` e variável especial `ans` para último resultado.
    \item Tratamento de erros com linha e coluna, recuperação básica de fluxo.
    \item Conversão interna de numerais romanos via rotina dedicada (`rmntoi`).
    \item Comandos de saída: `exit`, `quit`, `q`.
    \item Empacotamento automático de fontes via alvo `make compact` (gera `.tar.gz`).
\end{itemize}
...
A gramática adotada é equivalente à apresentada em aula, com níveis para \textit{expressão}, \textit{termo} e \textit{fator}. Em EBNF simplificada:
\begin{align*}
...
O parser segue padrão LL(1), lendo tokens da esquerda para a direita e construindo derivações mais à esquerda por meio de chamadas recursivas. O primeiro token é adiantado em \texttt{main.c}, e cada chamada de \texttt{expression}, \texttt{cmd} ou \texttt{match} garante que a condição LL(1) seja preservada.
...
\chapter{Empacotamento e Release}
O projeto inclui um fluxo simples de empacotamento:
\begin{enumerate}
    \item \textbf{Build}: `make mybc` compila o executável.
    \item \textbf{Compactação}: `make compact` gera arquivo `mybc_grp9_YYYYMMDD.tar.gz` com fontes e Makefile.
    \item \textbf{Tag opcional}: em repositório Git, criar tag anotada (`git tag -a v1.0.0 -m "Release v1.0.0"`).
    \item \textbf{Publicação}: anexar o `.tar.gz` à página de releases (GitHub) ou distribuir internamente.
\end{enumerate}
...
\chapter{Correções Recentes}
\begin{itemize}
    \item \textbf{Lexer de Romanos}: correção de loops que atribuíam resultado booleano de comparação a `lexeme[i]` (parênteses ausentes), causando perda de caracteres e avaliação incorreta (`MD` retornava 1000). Agora numerais são capturados integralmente e convertidos corretamente (e.g., `MD` → 1500, `CDXLIV` → 444).
    \item \textbf{Documentação}: adicionada seção de release e resumo de funcionalidades.
    \item \textbf{Exemplos}: sessão atualizada demonstrando uso de `ans` e numerais romanos.
\end{itemize}
